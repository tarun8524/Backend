...
class GetDBdataforintrusion():
    ...
    def get_max_intrusion_camera(self):
        ...
        if result:
            return MaxIntrusion(camera_name= result[0]["_id"] , count = result[0]["count"])# {"camera_name": result[0]["_id"], "count": result[0]["count"]}
        else:
    ...
    def get_intrusion_trend(self):
        ...
        # Return only the hours found in DB
        # hourly_data = [{"hour": f"{r['_id']:02d}:00", "count": r["count"]} for r in results]
